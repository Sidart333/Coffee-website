...
if(isset($_POST['place_order'])){
    ...
    $addrss=$_POST['flat'].','.$_POST['street'].','.$_POST['city'].','.$_POST['country'].','.
    $_POST['pincode'];
    $addrss=filter_var($addrss,FILTER_SANITIZE_STRING);
    ...
    if(isset($_GET['get_id'])){
        ...
        if($get_products->rowCount()>0){
            while($fetch_p=$get_product->fetch(PDO :: FETCH_ASSOC)){
                ...
                $insert_order->execute([unique_id(),$user_id,$name,$number,$email,$addrss,$address_type,$method,$fetch_p['id'],$fetch_p['price'],1]);
        ...
        while($f_cart=$varify_cart->fetch(PDO :: FETCH_ASSOC)){
            $insert_order=$conn->prepare("INSERT INTO `orders`(id,user_id,name,number,email,address,address_type,$method,product_id,price,qty)VALUES(?,?,?,?,?,?,?,?,?,?,?)");
                $insert_order->execute([unique_id(),$user_id,$name,$number,$email,$addrss,$address_type,$method,$fetch_p['id'],$fetch_cart['price'],$fetch_cart['qty']]);
                header('location: order.php');
...
<head>
    ...
    <title>Coffee Website - checkout page</title>
</head>
    ...
    <div class="banner">
        ...
        <section class="checkout">
            <div class="title">
                ...
                <h1>checkout summary</h1>
                <p>Lorem ipsum dolor sit, amet consectetur adipisicing elit. Odio nostrum praesentium rerum corporis, fugiat autem iusto quo sint cumque facere molestiae doloribus tenetur quos dolorem repudiandae sapiente eius nam</p>
            </div>
                <div class="row">
                    <form method="post">
